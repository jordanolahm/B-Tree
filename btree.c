...
*/
bt_node* allocate_btree_node (unsigned int order)
{
...
*       @return The allocated B-tree node
*/
static void free_btree_node (bt_node *node) {
...
static void btree_split_child(btree * btree, bt_node * parent,
				unsigned int index, bt_node * child)
{
	int i = 0;
...
insert:	i = node->nr_active - 1;
	...
	else
	{
		...
		if(child->nr_active == 2*btree->order - 1)
        {
            ...
            if(btree->value(key_val->key) > btree->value(node->key_vals[i]->key))
                i++;
		}
...
{
    bt_node *rnode = btree->root;

    if(rnode->nr_active == (2*btree->order - 1))
    {
		bt_node *new_root = allocate_btree_node(btree->order);
...
del_loop:
    ...
    //If the leaf node is the root permit deletion even if the number of keys is less than (t - 1)
    ...
    if(node->leaf == false)
    {
        ...
        if ((node->children[index + 1]->nr_active > btree->order - 1) )
        {
        ...
        {
            comb_node = merge_nodes(btree,node->children[index], node->key_vals[index], node->children[index + 1]);
    ...
    // we encounter on the way has atleast 't' (order of the tree) keys
    if(node->leaf && (node->nr_active > btree->order - 1))
    {
        node_pos.node  = node;
        node_pos.index = index;
...
static void copy_key_val(btree *btree, bt_key_val *src, bt_key_val *dst)
{
    ...
    memcpy(src->key, dst->key, keysize);

    if(src->val)
    {
        datasize = btree->data_size(src->val);
        dst->val = (void *) malloc(datasize);
        memcpy(src->val, dst->val, datasize);
    }
}
...
{
    node_pos node_pos = get_max_key_pos(btree, btree->root);
...
{
    node_pos node_pos = get_min_key_pos(btree,btree->root);
...
void print_single_node(btree *btree, bt_node *node)
{
    ...
    {
        printf("0x%x(%d) ", btree->value(node->key_vals[i]->key), node->level);
        i++;
    }
